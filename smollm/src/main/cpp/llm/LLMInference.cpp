...
LLMInference::loadModel(const char* model_path, float minP, float temperature, bool storeChats, long contextSize) {
    LOGi("loading model with"
         ...
         "\n\tstoreChats = %d"
         "\n\tcontextSize = %li",
         model_path, minP, temperature, storeChats, contextSize);
...
LLMInference::startCompletion(const char* query) {
    ...
    int newLen = llama_chat_apply_template(tmpl, _messages.data(), _messages.size(), true, _formattedMessages.data(),
    ...
    if (newLen > (int)_formattedMessages.size()) {
        ...
        newLen = llama_chat_apply_template(tmpl, _messages.data(), _messages.size(), true, _formattedMessages.data(),
                                           _formattedMessages.size());
    }
    if (newLen < 0) {
